...
public class Lambda_01_Test {
    ...
    public void test_filter_by_age() throws Exception {
        ...
        List<Person> result = personList.stream().filter(person -> person.getAge() >= 18).collect(Collectors.toList());
    ...
    public void test_filter_by_firstname() throws Exception {
        ...
        List<Person> result = personList.stream().filter(person -> person.getFirstname().equals("first_10")).collect(Collectors.toList());
    ...
    public void test_filter_by_password() throws Exception {
        ...
        List<Person> result = personList.stream().filter(person ->
                person.getAge() > 49 && DigestUtils.sha512Hex(person.getPassword()).equals(passwordSha512Hex)
